...
const PitchDetail = () => {
  ...
  if (!pitch) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">Không tìm thấy thông tin sân</h2>
        <Button variant="primary" onClick={() => navigate('/tim-san')}>Quay lại tìm sân</Button>
      </div>
    );
  }
  ...
  const handleBooking = () => {
    ...
    alert(`Đang tiến hành đặt cọc cho khung giờ: ${selectedSlot.startTime} - ${selectedSlot.endTime}\nGiá: ${selectedSlot.price.toLocaleString('vi-VN')}đ`);
  };
    ...
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 mb-8">
        ...
        <p className="text-gray-600 bg-gray-50 p-4 rounded-lg">
          * Mô tả chi tiết sân sẽ được hiển thị ở đây. Hệ thống chiếu sáng đạt chuẩn, mặt cỏ nhân tạo chất lượng cao, có khu vực căng tin và bãi giữ xe rộng rãi.
        </p>
      </div>
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
        ...
        {}
        <div className="mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-2">Ngày đặt sân</label>
          <input 
            type="date" 
            value={selectedDate}
            onChange={(e) => {
        ...
        {availableSlots.length > 0 ? (
            ...
            {availableSlots.map((slot) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-sm mt-1 ${slot.isBooked ? 'text-gray-400' : isSelected ? 'text-blue-600 font-medium' : 'text-green-600'}`}>
          ...
          <div className="text-center py-10 bg-gray-50 rounded-xl mb-8">
            <p className="text-gray-500">Chưa có lịch trống nào được cập nhật cho ngày này.</p>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6 border-t border-gray-100">
          <div className="text-left w-full sm:w-auto">
            ...
            <p className="text-2xl font-bold text-blue-600">
              {selectedSlot ? `${(selectedSlot.price * 0.3).toLocaleString('vi-VN')}đ` : '0đ'}
            </p>
          </div>
          <Button 
            variant="primary" 
