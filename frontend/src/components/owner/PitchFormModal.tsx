...
const PitchFormModal = ({ isOpen, onClose, onSubmit, initialData }: PitchFormModalProps) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ name: '', type: 'FIVE_A_SIDE', status: 'Hoạt động', price: '' });
    }
  }, [initialData, isOpen]);
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              {/* Value đổi thành Enum chuẩn của Backend */}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Trạng thái (Tạm)</label>
              <select disabled className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 text-gray-500 outline-none cursor-not-allowed" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})}>
